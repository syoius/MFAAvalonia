...
public partial class RecordTaskViewModel : ObservableObject
{
    ...
    private string? _recordingInstanceId;
    ...
    // 直接转发到当前实例的 TaskQueueViewModel，避免重复逻辑。
    ...
    private async Task StartRecordingAsync()
    {
        try
        {
            ...
            var activeProcessor = ActiveProcessor;
            if (activeProcessor == null)
            {
                ToastHelper.Error("未找到当前实例，无法开始录制");
                return;
            }
            _recordingInstanceId = activeProcessor.InstanceId;
    ...
    private async Task ExecuteActionAsync(string? actionName)
    {
        ...
        try
        {
            ...
            var processor = GetRecordingProcessor();
            if (processor == null)
            {
                ToastHelper.Error("未找到录制实例，请重新开始录制");
                return;
            }

            var tasker = await processor.GetTaskerAsync();
            if (tasker == null)
            {
    ...
    private MaaProcessor? GetRecordingProcessor()
    {
        if (string.IsNullOrWhiteSpace(_recordingInstanceId))
        {
            return ActiveProcessor;
        }

        return MaaProcessorManager.Instance.TryGetInstance(_recordingInstanceId, out var processor)
            ? processor
            : null;
    }

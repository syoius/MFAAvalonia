...
public partial class CopilotViewModel : ObservableObject
{
    ...
    private static readonly ObservableCollection<object> EmptyDevices = new();
    private static readonly ObservableCollection<LogItemViewModel> EmptyLogs = new();

    private static TaskQueueViewModel? ActiveTaskViewModel =>
        Instances.InstanceTabBarViewModel.ActiveTab?.TaskQueueViewModel;
    ...
    private static string GetActiveResourceBase()
    {
        try
        {
            ...
            var name = taskVm.CurrentResource;
            var selected = taskVm.CurrentResources?.FirstOrDefault(r => r.Name == name);
    ...
    // 实现采取最小必要封装，直接转发到当前实例的 TaskQueueViewModel，避免重复逻辑。

    // 显示控制（与主页一致）
    public int ShouldShow
    {
        get => ActiveTaskViewModel?.ShouldShow ?? 0;
        set
        {
            if (ActiveTaskViewModel != null)
            {
                ActiveTaskViewModel.ShouldShow = value;
            }
        }
    }
    ...
    {
        get => ActiveTaskViewModel?.Devices ?? EmptyDevices;
        set
        {
            if (ActiveTaskViewModel != null)
            {
                ActiveTaskViewModel.Devices = value;
            }
        }
    }
    ...
    {
        get => ActiveTaskViewModel?.CurrentDevice;
        set
        {
            if (ActiveTaskViewModel != null)
            {
                ActiveTaskViewModel.CurrentDevice = value;
            }
        }
    }
    ...
    {
        get => ActiveTaskViewModel?.CurrentController ?? MaaControllerTypes.None;
        set
        {
            if (ActiveTaskViewModel != null)
            {
                ActiveTaskViewModel.CurrentController = value;
            }
        }
    }
    ...
    public ObservableCollection<LogItemViewModel> LogItemViewModels =>
        ActiveTaskViewModel?.LogItemViewModels ?? EmptyLogs;
    ...
    private void CustomAdb()
    {
        ...
        try { ActiveTaskViewModel?.CustomAdbCommand?.Execute(null); }
    ...
    private void Clear()
    {
        try { ActiveTaskViewModel?.ClearCommand?.Execute(null); }
        catch (Exception ex) { LoggerHelper.Error(ex); }
    }
    ...
    private void Export()
    {
        try { ActiveTaskViewModel?.ExportCommand?.Execute(null); }
        catch (Exception ex) { LoggerHelper.Error(ex); }
    }
...
// 替换文件名中的非法字符
    ...
    public async Task LoadSelectedAsync()
    {
        ...
        try
        {
            ...
            // 重载资源
            var ok = ReloadResourcesForCopilot();
    ...
    public async Task UnloadActiveJobAsync()
    {
        try
        {
            ...
            ClearCopilotActiveDir();
            var reloadOk = ReloadResourcesForCopilot();
    ...
    private static bool ReloadResourcesForCopilot()
    {
        try
        {
            var processor = ActiveProcessor;
            var taskVm = ActiveTaskViewModel;
            if (processor == null || taskVm == null)
            {
                return false;
            }
            ...
            var currentTasks = DispatcherHelper.RunOnMainThread(() =>
                new Collection<DragItemViewModel>(taskVm.TaskItemViewModels.ToList()));
            var ok = processor.InitializeData(currentTasks);
            DispatcherHelper.RunOnMainThread(() =>
            {
                if (Instances.InstanceTabBarViewModel.ActiveTab?.View is MFAAvalonia.Views.Pages.TaskQueueView view)
                {
                    view.ResetOptionPanels();
                }
            });
            return ok;
        }
...
partial class CopilotViewModel
{
    ...
    public Task RefreshActiveJobAsync()
    {
        return UpdateActiveJobFromDiskAsync();
    }
}
